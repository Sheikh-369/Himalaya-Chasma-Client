...
interface Props {
  product: IProductData;
  onClose: () => void;
  // Syncing with the thunk: sends the data and the new gallery files
  onUpdate: (data: IProductData, files?: File[]) => void;
}
...
export default function UpdateProductModal({ product, onClose, onUpdate }: Props) {
  // Helper to ensure data structure consistency when modal opens
  const sanitizeFrameData = (p: IProductData) => {
    const features = Array.isArray(p.features) 
      ? p.features 
      : typeof p.features === 'string' 
  ...
  // Sync state if the product prop changes
  useEffect(() => {
    setForm(sanitizeFrameData(product));
  }, [product]);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (selectedFiles.length > 4) {
        return setError('You can only upload a maximum of 4 images.');
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onUpdate(finalData, files.length > 0 ? files : undefined);
  };
      ...
      <div className="bg-[#0F1B35] border border-white/10 rounded-2xl w-full max-w-xl shadow-2xl overflow-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3 text-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            <div>
              <label className="block text-white/60 mb-1">Rating</label>
              <input type="number" name="rating" value={form.rating} onChange={handleChange} step={0.1} min={0} max={5} className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-3 py-2 outline-none" />
            </div>
          </div>

          {/* Reviews, Category, Badge */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div>
              <label className="block text-white/60 mb-1">Reviews</label>
              <input type="number" name="reviews" value={form.reviews} onChange={handleChange} min={0} className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-3 py-2 outline-none" />
            </div>
            <div>
              <label className="block text-white/60 mb-1">Category</label>
              <select name="category" value={form.category} onChange={handleChange} className="w-full bg-[#0F1B35] border border-white/10 text-white rounded-xl px-3 py-2 outline-none focus:border-secondary/50">
                {categories.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-white/60 mb-1">Badge</label>
              <input type="text" name="badge" value={form.badge || ''} onChange={handleChange} placeholder="e.g. New, Sale" className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-3 py-2 outline-none" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea name="description" value={form.description || ''} onChange={handleChange} rows={2} className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-3 py-2 outline-none focus:border-secondary/50" />
          </div>

          {/* Gallery & Alt */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div>
              <label className="block text-white/60 mb-1">Update Gallery (Max 4)</label>
              <input type="file" multiple accept="image/*" onChange={handleFileChange} className="w-full text-white/70 text-xs file:mr-4 file:py-1 file:px-3 file:rounded-full file:border-0 file:bg-secondary/20 file:text-secondary cursor-pointer" />
              <p className="text-[10px] text-white/40 mt-1">{files.length > 0 ? `${files.length} new files selected` : "Leave empty to keep current images"}</p>
            </div>
            <div>
              <label className="block text-white/60 mb-1">Alt text</label>
              <input type="text" name="alt" value={form.alt || ''} onChange={handleChange} className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-3 py-2 outline-none" />
            </div>
          </div>
          ...
          <div>
            <label className="block text-white/60 mb-1">Features (comma-separated)</label>
            <textarea name="features" value={Array.isArray(form.features) ? form.features.join(',') : ''} onChange={handleChange} rows={2} className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-3 py-2 outline-none" />
          </div>
          ...
          <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
              {form.frameDetails?.map((detail, idx) => (
                <div key={detail.label} className="space-y-1">
                  <span className="text-[10px] uppercase text-white/40 ml-1">{detail.label}</span>
                  <input type="text" value={detail.value} onChange={(e) => handleFrameChange(idx, e.target.value)} className="w-full bg-white/5 border border-white/10 text-white rounded-lg px-3 py-1.5 text-xs outline-none focus:border-secondary/50" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-3 py-2 rounded-xl border border-white/10 text-white/60 hover:bg-white/5">Cancel</button>
            <button type="submit" className="flex-1 px-3 py-2 rounded-xl bg-secondary text-primary font-bold hover:opacity-90">Update Product</button>
          </div>
        </form>
      </div>
    </div>
