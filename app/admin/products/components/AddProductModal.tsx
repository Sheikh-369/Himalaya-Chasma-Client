...
interface Props {
  onClose: () => void;
  onSubmit: (data: IProductData, files?: File[]) => void;
}
...
export default function AddProductModal({ onClose, onSubmit }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.name.trim()) return setError('Product name is required.');
    if (!form.price || form.price <= 0) return setError('Enter valid price.');
    if (files.length === 0) return setError('Please upload at least one image for the gallery.');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 bg-black/70 backdrop-blur-sm">
      <div className="bg-[#0F1B35] border border-white/10 rounded-2xl w-full max-w-xl shadow-2xl overflow-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3 text-sm">
          {error && <p className="text-red-400 bg-red-400/10 p-2 rounded-lg text-center font-medium">{error}</p>}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            ...
            <div>
              <label className="block text-white/60 mb-1">Badge</label>
              <input type="text" name="badge" value={form.badge || ''} onChange={handleChange} placeholder="e.g. New, Sale"
                className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-3 py-2 focus:outline-none focus:border-secondary/50" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div>
              <label className="block text-white/60 mb-1">Gallery Images (Max 4) *</label>
              <input 
                type="file" 
                name="gallery" 
                multiple 
                ...
                className="w-full text-white/70 text-sm file:mr-4 file:py-1.5 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-secondary/20 file:text-secondary hover:file:bg-secondary/30 cursor-pointer" 
              />
              <p className="text-[10px] text-white/40 mt-1">{files.length} files selected</p>
            </div>
            <div>
              <label className="block text-white/60 mb-1">Alt text</label>
              <input type="text" name="alt" value={form.alt || ''} onChange={handleChange}
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <textarea
                ...
                onChange={handleChange}
                rows={2}
                placeholder="Lightweight, UV400 Protection, Polarized..."
                className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-3 py-2 transition-all focus:outline-none focus:border-secondary/50"
              />
            </div>
          ...
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-3 py-2 rounded-xl border border-white/10 text-white/60 text-sm hover:bg-white/5 transition-colors">
              Cancel
            </button>
            <button type="submit" className="flex-1 px-3 py-2 rounded-xl bg-secondary text-primary text-sm font-bold hover:opacity-90 transition-opacity">
              Add Product
            </button>
          </div>
        </form>
