...
export default function AdminProductsPage() {
  ...
  const handleAddSubmit = (data: IProductData, files?: File[]) => {
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4 sm:px-6 py-5 border-b border-white/8">
            ...
            <div className="relative w-full md:w-auto">
              <input
                ...
                className="bg-white/5 border border-white/10 text-white text-sm rounded-xl pl-9 pr-4 py-2.5 w-full md:w-64"
              />
            </div>
          </div>
            ...
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className="border-b border-white/6 bg-white/[0.02]">
                  <th className="text-left px-6 py-4 text-white/40 text-xs font-semibold uppercase">Image</th>
                  <th className="text-left px-4 py-4 text-white/40 text-xs font-semibold uppercase">Product</th>
                  <th className="text-left px-4 py-4 text-white/40 text-xs font-semibold uppercase">Category</th>
                  <th className="text-left px-4 py-4 text-white/40 text-xs font-semibold uppercase">Price</th>
                ...
                {filtered.map((product) => (
                  <tr key={product.id} className="hover:bg-white/2 transition-colors">
                    <td className="px-6 py-4">
                      <img src={typeof product.image === 'string' ? product.image : ''} alt="" className="w-12 h-12 object-cover rounded-lg" />
                    </td>
                    <td className="px-4 py-4 text-white text-sm font-medium">{product.name}</td>
                    <td className="px-4 py-4 text-white/60 text-xs uppercase">{product.category}</td>
                    <td className="px-4 py-4 text-white text-sm font-semibold">Rs. {product.price.toLocaleString()}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => openEdit(product)} className="p-2 text-white/40 hover:text-secondary">
                        ...
                        <button onClick={() => setDeleteProductId(product.id!)} className="p-2 text-white/40 hover:text-red-400">
      ...
      {editProduct && (
        <UpdateProductModal
          ...
          onUpdate={(data, files) => {
            ...
            // Now passing the object containing main and gallery files
            dispatch(updateProduct(editProduct.id, data, files));
            closeEdit();
          }}
        />
      )}
